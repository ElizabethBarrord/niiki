...
//Rare exceptions to indicate a bug in the _niiki code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_niiki = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_niiki]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.niiki.module"
